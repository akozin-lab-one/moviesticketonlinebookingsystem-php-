...
@section('title','cinemaroomseatList')

@section('mainContent')
<h1 class="text-2xl font-semibold mb-4">Cinemas</h1>
<table class="min-w-full bg-white text-black border border-gray-300">
    <thead>
        <tr>
            ...
            <th class="py-2 px-4 border-b">Seat Type</th>
        ...
        @if (count($roomseats) !== 0)
            @foreach ($roomseats as $roomseat)
                <tr>
                    ...
                    <td class="text-center py-2 px-3 border-b">{{ $roomseat->seat_no }}</td>
                    <td class="text-center py-2 px-3 border-b">{{ $roomseat->row_name }}</td>
                    <td class="text-center py-2 px-3 border-b">{{ $roomseat->seat_type }}</td>
                    <td class=" text-center  py-2 px-2 border-b">
                        <form action="{{route('Admin#editRoomseatPage',$roomseat->id)}}" method="post">
                            @csrf
                            <button class="bg-blue-500 text-white px-4 py-1 rounded-md" type="submit">Edit</button>
                        </form>
                    </td>
                    <td class=" text-center  py-2 px-2 border-b">
                        <form action="{{route('Admin#deletelist', $roomseat->id)}}" method="post">
                            @csrf
...
<!-- Create Button -->
<div class="mt-4">
    <a href="{{route('Admin#createseatpage')}}"><button class="bg-blue-500 text-white py-2 px-4 rounded">Create
            CinemasRoomSeats</button></a>
</div>
