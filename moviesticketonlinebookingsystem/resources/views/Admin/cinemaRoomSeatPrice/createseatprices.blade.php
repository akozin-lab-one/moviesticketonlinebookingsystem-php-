...
@section('title','createcinemasroomseatprices')
...
<div class="container mx-auto grid place-items-center h-screen">
    <div class="border p-3 w-[40%]">
        ...
        @endif
        <h2 class="text-center font-extrabold">Create CinemasRoomsSeatPrices</h2>
        <form action="{{route('Admin#seatpricecreation')}}" method="post">
            @csrf
            <div>
                ...
                <select class="block w-[100%] h-[33px] rounded-lg" name="rowName" id="">
                    ...
                    @if(count($row_names) !== 0)
                        @foreach($row_names as $row)
                            <option value="{{$row->id}}">{{$room->row_name}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
            <div>
                ...
                <input class="block w-[100%] h-[33px] rounded-lg" type="text" name="roomSeatPrice" id="">
            </div>
            <div class="my-2">
